#!/bin/sh
# Thanks, http://lipidity.com/apple/cleaning-apps-mac-os-x/
help(){
...
echo " ==  Stripping debug symbols...";file -f found.tmp -kn | grep Mach | sed -E 's/(\(for architecture [[:alnum:]]+\))?:[[:space:]]*(setuid )?Mach-O.*//g' | while read LINE;do b_path=`dirname "$LINE"`;b_name=`basename "$LINE"`;eval cd "'$b_path'";eval /System/Library/PrivateFrameworks/DevToolsCore.framework/Versions/A/Resources/pbxcp -resolve-src-symlinks -strip-debug-symbols "'$b_name'" ..;eval mv "'../$b_name'" .;[ "$otpt" ]&&echo "stripped $LINE";eval cd "'$here'"
done;}
